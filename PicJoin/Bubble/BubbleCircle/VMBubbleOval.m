...
@interface VMBubbleOval ()

@property (nonatomic, strong) VMFrameOval *frameOval;
@property (nonatomic, strong) VMArrowBorderOval *arrowBorder;
@property (nonatomic, strong) VMArrowOval *arrow;
...
-(void)initBubbleOval
{
    ...
    self.frameOval = [[VMFrameOval alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    ...
    self.arrowBorder = [[VMArrowBorderOval alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width/3, self.frame.size.height)];
    
    [self.arrowBorder setBackgroundColor:self.backgroundColor];
    [self.arrowBorder setBoderColor:self.boderColor];
    [self.arrowBorder setType:self.typeOfBubble];
    
    self.arrow = [[VMArrowOval alloc] initWithFrame:CGRectMake(0, 0,self.frame.size.width/3, self.frame.size.height)];
    ...
    [self.arrowBorder.layer setAnchorPoint:CGPointMake(0.5, 1)];
    ...
    [self.arrowBorder setCenter:CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2)];
    [self.arrow setCenter:CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2)];
    
    self.arrowBorder.transform = CGAffineTransformMakeRotation(DEGREES_TO_RADIANS(200));
    ...
    [self addSubview:self.arrowBorder];
...
-(void)moveArrowWithCenterPanView:(CGPoint)center
{
    ...
    if (self.typeOfBubble == 1 || self.typeOfBubble == 3 ||self.typeOfBubble == 4 ||self.typeOfBubble == 5 || self.typeOfBubble == 7 ) {
        ...
        [self.arrowBorder setTransform:CGAffineTransformConcat(scale, rotate)];
    ...
    if (self.typeOfBubble == 2|| self.typeOfBubble == 6) {
        ...
        self.arrowBorder.bounds = bounds;
        [self.arrow setNeedsDisplay];
        [self.arrowBorder setNeedsDisplay];
        
        [self.arrow setTransform:rotate];
        [self.arrowBorder setTransform:rotate];
...
-(void)reloadView
{
    ...
    [self.delegate changeAllViewWheResize:self withCenter:[self.superview convertPoint:ct fromView:self]];
...
{
    CGPoint ct = [self convertPoint:self.panGetLocationView.center fromView:self.arrow];
    [self.delegate getCenterArrowInView:self withCenter:[self.superview convertPoint:ct fromView:self]];
}
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:
        {
            if([pan locationInView:self].x >= minimumResize && [pan locationInView:self].y >= minimumResize)
            {
                ...
                [self.arrowBorder setCenter:CGPointMake(width/2, height/2)];
