...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(dismissAllModalDialog)
                                                 name:@"dismissallmodaldialog" object:nil];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //AnhTN update 24/09/2013: fix bug for iOS 7
...
//        self.navigationController.navigationBar.titleTextAttributes = [NSDictionary dictionaryWithObject:self.navigationController.navigationBar.tintColor forKey:UITextAttributeTextColor];
//    }
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if([title localizedCompare:NSLocalizedStringFromTable(@"Copy Link", @"SVWebViewController", @"")] == NSOrderedSame) {
        ...
        if (self.mainWebView.request.URL.absoluteString.length && self.mainWebView.request.URL.absoluteString.length > 0) {
